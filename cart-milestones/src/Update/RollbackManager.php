...
class RollbackManager {
		...
		if ( null === $entry || empty( $entry['zip_url'] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Version %s is not available for rollback.', 'cart-milestones' ),
					$version
				)
			);
		}
	...
	public function rollback_previous(): true|\WP_Error {
		...
		if ( null === $previous ) {
			return new \WP_Error(
				...
				__( 'No previous version available for rollback.', 'cart-milestones' )
			);
		}
	...
	private function install_from_url( string $zip_url, string $version ): true|\WP_Error {
		...
		if ( ! \WP_Filesystem() ) {
			return new \WP_Error( 'cm_rollback_filesystem', __( 'Could not initialise filesystem.', 'cart-milestones' ) );
		}
		...
		if ( false === $result ) {
			return new \WP_Error( 'cm_rollback_failed', __( 'Rollback installation failed.', 'cart-milestones' ) );
		}
